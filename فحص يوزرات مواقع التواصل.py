try:
    ...
    Y   = Fore.YELLOW
    b   = Fore.BLUE
    lib = input(Y+'''[1]لتحميل المكاتب تلقائيا
[2]للدخول للاداة او محمل المكاتب
اختر واحده منهن ==>
    ''')
    if lib =='1':
...
""")
print(b+"telegram-->@OoOoO2oO"+'\n'+"="*40)
user = input(G+"ادخل اليوزر==>")
...
def TikTok():
        ...
        if re.status_code == 404:
            print(G+f"[+] [ TikTok ] [✔️] ==> متاح[{user}]")
            Account = (f'https://api.telegram.org/bot{tokan}/sendMessage?chat_id={ID}&text=𝐔𝐒𝐄𝐑 𝐍𝐄𝐖 tiktok\n------------------------------\n𝐔𝐒𝐄𝐑-> {user}\n--------------------------------\ntele==>@T3bsBoT ~ @OoOoO2oO')
            k = requests.post(Account) 
            Status = 'OFF'
        elif re.status_code == 200:
            print(R+f"[-] [ TikTok ] [❌] ==> غير متاح[{user}]")
...
def Tellonym():
        ...
        re = rs.get(url, headers=headers)
        if re.status_code == 404:
            print(G+f"[+] [ Tellonym ] [✔️] ==> متاح [{user}]")
            Account = (f'https://api.telegram.org/bot{tokan}/sendMessage?chat_id={ID}&text=𝐔𝐒𝐄𝐑 𝐍𝐄𝐖 Tellonym\n------------------------------\n𝐔𝐒𝐄𝐑-> {user}\n--------------------------------\ntele==>@T3bsBoT ~ @OoOoO2oO')
            k = requests.post(Account) 
            Status = 'OFF'
        elif re.status_code == 200:
            print(R+f"[-] [ Tellonym ] [❌] ==> غير متاح [{user}]")
...
def GitHub():
        ...
        re = rs.get(url, headers=headers)
        if re.status_code == 404:
            print(G+f"[+] [ GitHib ] [✔️] ==> متاح [{user}]")
            Account = (f'https://api.telegram.org/bot{tokan}/sendMessage?chat_id={ID}&text=𝐔𝐒𝐄𝐑 𝐍𝐄𝐖 GitHub\n------------------------------\n𝐔𝐒𝐄𝐑-> {user}\n--------------------------------\ntele==>@T3bsBoT ~ @OoOoO2oO')
            k = requests.post(Account) 
            Status = 'OFF'
        elif re.status_code == 200:
            print(R+f"[-] [ GitHub ] [❌] ==> غير متاح [{user}]")
...
def Twitch():
        ...
        if re.status_code == 404:
            if len(user) >= 4:
                print(G+f"[+] [ Twitch ] [✔️] ==> متاح [{user}]")
                Account = (f'https://api.telegram.org/bot{tokan}/sendMessage?chat_id={ID}&text=𝐔𝐒𝐄𝐑 𝐍𝐄𝐖 Twitch\n------------------------------\n𝐔𝐒𝐄𝐑-> {user}\n--------------------------------\ntele==>@T3bsBoT ~ @OoOoO2oO')
                k = requests.post(Account)
                Status = 'OFF'
            else:
                print(Y+f"[-] [ Twitch ] [❌] ==> Less Than 4 [{user}]")
        elif re.status_code == 200:
            print(R+f"[-] [ Twitch ] [❌] ==> غير متاح [{user}]")
...
def Twitter():
        ...
        if ('error') in re:
            print(G+f"[+] [ Twitter ] [✔️] ==> متاح [{user}]")
            Account = (f'https://api.telegram.org/bot{tokan}/sendMessage?chat_id={ID}&text=user Twitter\n------------------------------\n𝐔𝐒𝐄𝐑-> {user}\n--------------------------------\ntele==>@T3bsBoT ~ @OoOoO2oO')
            k = requests.post(Account) 
        else:
            print(R+f"[-] [ Twitter ] [❌] ==> غير متاح [{user}]")
...
def Reddit():
        ...
        if re.status_code == 404:
            if len(user) >= 3:
                print(G+f"[+] [ Reddit ] ==> متاح [{user}] ")
                Account = (f'https://api.telegram.org/bot{tokan}/sendMessage?chat_id={ID}&text=𝐔𝐒𝐄𝐑 𝐍𝐄𝐖 Reddit\n------------------------------\n𝐔𝐒𝐄𝐑-> {user}\n--------------------------------\ntele==>@T3bsBoT ~ @OoOoO2oO')
                k = requests.post(Account)
                Status = 'OFF'
            else:
            	print(R+f"[-] [ Reddit ] ==> غير متاح[{user}] ")
...
def Instagram():
        ...
        if (
            ...
            print(G+f"[-] [ Instagram ] [✔️] ==> متاح [{user}] ")
            Account = (f'https://api.telegram.org/bot{tokan}/sendMessage?chat_id={ID}&text=𝐔𝐒𝐄𝐑 𝐍𝐄𝐖 instagram\n------------------------------\n𝐔𝐒𝐄𝐑-> {user}\n--------------------------------\ntele==>@T3bsBoT ~ @OoOoO2oO')
            k = requests.post(Account) 
            Status = 'OFF'

        else:
            print(R+f"[-] [ Instagram ] [❌] ==> غير متاح [{user}] ")
def SnapChat():
        ...
        if re.text.find('"status_code":"OK"') >= 0:
            print(G+f"[+] [ SnapChat ] [✔️] ==> متاح [{user}]")
            Account = (f'https://api.telegram.org/bot{tokan}/sendMessage?chat_id={ID}&text=user snap\n------------------------------\n𝐔𝐒𝐄𝐑-> {user}\n--------------------------------\ntele==>@T3bsBoT ~ @OoOoO2oO')
            k = requests.post(Account) 
        elif re.text.find('"status_code":"TAKEN"') >= 0:
            print(R+f"[-] [ SnapChat ] [❌] ==> غير متاح [{user}]")

TikTok();print("•"*45);SnapChat();print("•"*45);Instagram();print("•"*45);Twitter();print("•"*45);Tellonym();print("•"*45);GitHub();print("•"*45);Reddit();print("•"*45);Twitch();print("•"*45);print("="*40);print('telegram ');print('@OoOoO2oO ');print('@T3bsBoT ');print("="*40)